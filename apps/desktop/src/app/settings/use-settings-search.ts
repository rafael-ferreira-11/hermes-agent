...
export function useSettingsSearchCatalog(enabled: boolean) {
  ...
  // v0: hidden tabs (Gateways — billing and providers have no subpages) stay out of
  // the palette's settings search; their pages still resolve when a flow deep-links
  // straight to them.
  const HIDDEN_SEARCH_VIEWS = new Set<SettingsView>(['gateway'])
