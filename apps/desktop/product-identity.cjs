...
const variants = {
  ...
  // CommonAgent: this fork's product identity. Keeps the appId the first
  // CommonAgent installers shipped with so an installed app upgrades in
  // place instead of living next to its old self.
  commonagent: { display: 'CommonAgent', kebab: 'commonagent', pascal: 'CommonAgent', appId: 'com.commonagent.app' },
  light: {
...
if (!['', 'light', 'bundled', 'store', 'commonagent'].includes(variant)) {
  throw new Error(`Unknown HERMES_DESKTOP_VARIANT ${variant}. expected one of (empty), light, bundled, store, commonagent`)
}
...
const identity = {
  ...
  light,
  displayName,
  appId: name.appId ?? `com.nousresearch.${name.kebab}${kebabSuffix}`,
